...
def translate_and_save(df, abbreviations_on, source_language, target_language):
    ...
    translated_headers = [
        ...
        for header in tqdm(headers_to_translate, desc="Translating headers")
    ]

    plus_column_name = "Plus"
    if plus_column_name in df.columns:
        tqdm.pandas(desc="Translating Plus column")
        df[plus_column_name] = df[plus_column_name].progress_apply(
            lambda x: (
                GoogleTranslator(
...
def abbreviation_to_fullname(df, data_structure):
    ...
    print(
        "Abbreviations have been changed and the new data has been saved here:",
        output_file,
    )
    return df
...
def translate_dataset(
    abbreviations_on=True, source_language="fr", target_language="en"
):
